...
define(
    ...
    function (Base) {
        ...
        class Events extends Base {
            ...
            waitFor(event, requiredArgs, wait) {
                return this.remote
                    .setExecuteAsyncTimeout(wait)
                    .executeAsync(function (event, requiredArgs, done) {
                        var testNamespace  = window.sitecuesTestingNamespace,
                            eventNamespace = testNamespace[event];

                        if (!eventNamespace || !testNamespace) {
                            throw new Error('Sitecues testing namespace should have been initialized before waiting for event');
                        }
