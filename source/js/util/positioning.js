...
sitecues.def('util/positioning', function (positioning, callback, log) {
    ...
    sitecues.use('jquery', 'util/common', 'platform', 'conf', function ($, common, platform, conf) {
	    ...
	    positioning.getAllBoundingBoxes = function (selector, proximityBeforeBoxesMerged, stretchForSprites) {
		    ...
		    var clipRect = common.getAncestorClipRect($selector);
		    getAllBoundingBoxesExact($selector, allRects, clipRect, stretchForSprites);
		    common.combineIntersectingRects(allRects, proximityBeforeBoxesMerged); // Merge overlapping boxes
    ...
    function getAllBoundingBoxesExact($selector, allRects, clipRect, stretchForSprites) {
        $selector.each(function () {
          ...
          if (!isElement) {
            if (this.nodeType === 3 && $.trim(this.textContent) !== '') {
              // -- Acutal text rect --
              addRect(allRects, clipRect, common.getBoundingRectMinusPadding(this));
            }
            return true;
          }
          ...
          // --- Overflowing content ---
          addRect(allRects, clipRect, common.getOverflowRect(this, style));

          // --- Visible border ---
          if (common.hasVisibleBorder(style)) {
            addRect(allRects, clipRect, this.getBoundingClientRect()); // Make it all visible, including padding and border
            return true;
          }

          // --- List bullets ---
          addRect(allRects, clipRect, common.getBulletRect(this, style));


          // --- Background sprites ---
          if (stretchForSprites) {
            addRect(allRects, clipRect, common.getSpriteRect(this, style));
          }
      ...
      function addRect(allRects, clipRect, unclippedRect) {
        ...
        var rect = common.getClippedRect(unclippedRect, clipRect);
        if (rect.width < common.kMinRectWidth || rect.height < common.kMinRectHeight) {
