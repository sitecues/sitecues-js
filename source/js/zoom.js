...
sitecues.def('zoom', function (zoom, callback) {
  ...
  sitecues.use('jquery', 'conf', 'platform',
    function ($, conf, platform) {
      ...
      function shouldSmoothZoomOnInit() {
        return shouldSmoothZoom() && !platform.browser.isIE9;
      }
      ...
      function finishGlideIfEnough() {
        ...
        var zoomRemainingBeforeMin = MIN_ZOOM_PER_CLICK - Math.abs(midAnimationZoom - completedZoom),
          msBeforeZoomFinished = Math.max(0, zoomRemainingBeforeMin * MS_PER_X_ZOOM);

        minZoomChangeTimer = setTimeout(finishGlideEarly, msBeforeZoomFinished);
      }
      ...
      function beginZoomOperation(targetZoom) {
        ...
        if (completedZoom === 1) {   // Zooming up from 1 means we don't have any style fixes applied yet
          applyZoomCSSFixes();
        }
      ...
      // Apply CSS enhancements we use for zoom, that don't need to be animated
      ...
      function getZoomCss(targetZoom) {
        var transform = 'scale(' + targetZoom + ') ' + getFormattedTranslateX(targetZoom),
          css = {
            transform: transform
          };
      ...
      // It is a best practice to clean up after ourselves when the user turns off all zoom (back to 1x)
