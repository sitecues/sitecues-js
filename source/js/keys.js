/**
 * The file contains the logics related to key press window events.
 */
sitecues.def('keys', function (keys, callback) {
  ...
  var zoomKeysEnabled = true,
    ...
    // Define keys map used to bind actions to hotkeys key codes vary across browsers and
    // we need to support the numeric keypad. See http://www.javascripter.net/faq/keycodes.htm
    origTest = {
        ...
        if (has.call(origMap, key) && origTest[key](event)) {
          ...
          if (origMap[key].requiresMouseHighlightActive) {
            if (!mh.isActive()) {
              ...
              // We're going to attach the target dom element to the event, whether it's available or not.
