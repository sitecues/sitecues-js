...
sitecues.def('keys', function (keys, callback, log) {
  ...
  var has = Object.prototype.hasOwnProperty; // SERIOUSLY?!?!
  ...
  sitecues.use('jquery', 'mouse-highlight', 'util/common', function($, mh, common){
    ...
    keys.handle = function (key, event) {
      ...
      // Shim for browser short cuts
      // (None of our current features requires 'ctrl' or 'alt' so far.)
      if (event.ctrlKey || event.altKey) {
        return true;
      }
        
      // Emit event if defined
      if (key.event) {
    ...
    keys.hook = function (event) {
      ...
      for (key in keys.map){
        ...
        if (keys.map.hasOwnProperty(key)) {
