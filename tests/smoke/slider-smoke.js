...
describe("sitecues", function () {
  describe("navigator", function () {
    swdda.describeForEachBrowser("simple", function (session) {
     ...
     it("body should not have a zoom", function (done) {
        session.browser.elementByTagName('body', function(err, body) {
          session.browser.getComputedCss(body, 'zoom', function(err, value) {
     ...
     it("should move the mouse to the big A and click", function (done) {
        session.browser.elementByClassName('letterBig', function(err, letterBig) {
          session.browser.moveTo(letterBig, 1, 1, function(err) {
            session.browser.moveTo(letterBig, function(err) {
              session.browser.click(function(err) {
                done();
              });
            });
          });
     ...
     it("body should have a zoom", function (done) {
        session.browser.elementByTagName('body', function(err, body) {
          session.browser.getComputedCss(body, 'zoom', function(err, value) {
            // expect(value).to.equal('1.100000023841858');
            done();
          });
        });
      });
