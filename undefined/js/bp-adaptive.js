 ...
 */
sitecues.define("bp-adaptive/bp-adaptive", [ "run/bp/model/state", "run/bp/view/view", "run/bp/constants", "mini-core/native-global" ], function(state, bpView, BP_CONST, nativeGlobal) {
  var lastBgColor;
  ...
  function checkBackgroundColorChange(onPaletteUpdate, doForceBadgeUpdate) {
    var newBgColor = getBackgroundColor(), doBadgeUpdate = doForceBadgeUpdate;
    if (newBgColor !== lastBgColor) {
      lastBgColor = newBgColor;
      doBadgeUpdate = true;
    }
    if (doBadgeUpdate) {
      sitecues.require([ "page/util/color" ], function(colorUtil) {
        if (true) {
          console.log("Updating badge palette");
        }
  ...
  function adaptToSitecuesThemeChange(newTheme) {
    ...
    state.set("isAdaptivePalette", "dark" === newTheme);
...
//# sourceMappingURL=bp-adaptive.js.map